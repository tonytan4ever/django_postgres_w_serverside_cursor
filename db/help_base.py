...
from django.db.backends.postgresql_psycopg2.base import (DatabaseWrapper, DatabaseFeatures,
...
if float(str(Database.__version__)[:4]) < 2.4:
    raise ImportError("psycopg version %s is not supported. Has to be greater than 2.4"
...
class pgServerSideCursorWrapper(CursorWrapper):
    """CursorWrapper for pgServerSideCursorWrapper, this one needs to manage 
       a transaction internally, and needs to do a help_transaction.commint() 
       after each execute
    """
    def __init__(self, cursor, db_wrapper):
        """mannually enter the transaction cycle first"""
        super(pgServerSideCursorWrapper, self).__init__(cursor)
        self.db_wrapper = db_wrapper

    def execute(self, query, args=None):
        """mannually enter the transaction cycle first"""
        self.db_wrapper.enter_transaction_management(True)
        super(pgServerSideCursorWrapper, self).execute(query, args)

    def executemany(self, query, args=None):
        self.db_wrapper.enter_transaction_management(True)
        super(pgServerSideCursorWrapper, self).executemany(query, args)
